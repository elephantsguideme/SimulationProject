...
int main()
{
  ...
  while(command)       {
    ...
    if (blood_centre->get_system_time() == blood_expired->event_time)blood_expired->Execute();
    if (blood_centre->get_system_time() == normal_order_arrived->event_time)normal_order_arrived->Execute();
    if (blood_centre->get_system_time() == emergency_order_arrived->event_time)emergency_order_arrived->Execute();
    //std::cout << emergency_order_arrived->event_time;
    //checking contitions
    bool change;
    do{
      change = false;
    ...
    if (blood_expired->event_time < new_event_time && blood_expired->event_time>0) new_event_time = blood_expired->event_time;
    if (normal_order_arrived->event_time < new_event_time  && normal_order_arrived->event_time>0)new_event_time = normal_order_arrived->event_time;
    if (emergency_order_arrived->event_time < new_event_time  && emergency_order_arrived->event_time>0)new_event_time = emergency_order_arrived->event_time;
